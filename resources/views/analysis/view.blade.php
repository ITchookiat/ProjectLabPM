    ...
    <div class="content-header">
      ...
      <div class="container-fluid">
        <div class="row mb-0">
          <div class="col-sm-6">
            @if($type == 1)
              <h5>สัญญาเงินกู้รถยนต์ (Loan Agreement)</h5>
            @elseif($type == 3)
              <h5>สัญญาเงินกู้รถจักรยานยนต์ (P04 Loan Agreement)</h5>
            @endif
          </div>
          <div class="col-sm-6">
            @if($type == 1 or $type == 3)
    ...
    <div class="row">
      <div class="col-md-3">
        @if(auth::user()->type == 'Admin' or auth::user()->type == 'แผนก วิเคราะห์')
          <div class="row">
            <div class="col-md-6">
              <a href="{{ route('MasterAnalysis.index') }}?type={{2}}" class="btn btn-success btn-block mb-3">Compose</a>
            </div>
            <div class="col-md-6">
                ...
                <div class="float-right form-inline">
                  @if(auth::user()->type == "Admin" or auth::user()->type == "แผนก วิเคราะห์")
                    ...
                    <ul class="dropdown-menu" role="menu">
                      <li><a target="_blank" class="dropdown-item" href="{{ action('ReportAnalysController@ReportDueDate', 8) }}"> รายงาน ขอเบิกเงินประจำวัน</a></li>
                      <li class="dropdown-divider"></li>
                      <li><a target="_blank" class="dropdown-item" href="{{ action('ReportAnalysController@ReportDueDate', 1) }}"> รายงาน ขออนุมัติประจำวัน</a></li>
                      <li class="dropdown-divider"></li>
                      <li><a target="_blank" class="dropdown-item" href="{{ action('ReportAnalysController@ReportDueDate', 2) }}"> รายงาน เงินกู้รถยนต์</a></li>
                    </ul>
                  @endif
                              ...
                              @if($row->branch_car == 'ปัตตานี')
                                <tr>
                                  ...
                                  <td class="text-right">
                                    ...
                                    {{-- แก้ไข / ดูรายการ --}}
                                    @if(auth::user()->type == "Admin")
                                      <a href="{{ action('AnalysController@edit',[1,$row->id,$newfdate,$newtdate,$branch,$status]) }}" class="btn btn-warning btn-sm" title="แก้ไขรายการ">
                                        <i class="far fa-edit"></i>
                                      </a>
                                    @elseif(auth::user()->position == "MANAGER" or auth::user()->position == "AUDIT" or auth::user()->position == "MASTER" or auth::user()->position == "STAFF")
                                      @if($row->StatusApp_car == 'อนุมัติ')
                                        <a href="{{ action('AnalysController@edit',[2,$row->id,$newfdate,$newtdate,$branch,$status]) }}" class="btn btn-success btn-sm" title="ดูรายการ">
                                          <i class="fas fa-eye"></i>
                                        </a>
                                      @else
                                        <a href="{{ action('AnalysController@edit',[2,$row->id,$newfdate,$newtdate,$branch,$status]) }}" class="btn btn-warning btn-sm" title="แก้ไขรายการ">
                                          <i class="far fa-edit"></i>
                                        </a>
                                      @endif
                                    @endif
                                    ...
                                    @if(auth::user()->type == "Admin") 
                                      <form method="post" class="delete_form" action="{{ action('AnalysController@destroy',[$row->id, 1]) }}" style="display:inline;">
                                        {{csrf_field()}}
                                        <input type="hidden" name="_method" value="DELETE" />
                                        <button type="submit" data-name="{{ $row->Contract_buyer }}" class="delete-modal btn btn-danger btn-sm AlertForm" title="ลบรายการ">
                                          <i class="far fa-trash-alt"></i>
